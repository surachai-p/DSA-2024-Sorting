...
def compare_sort():
    ...
    for size in sizes:
        # สร้างชุดข้อมูลแบบสุ่มในช่วง 0-999999
        ...
        # วัดเวลา Merge Sort
        start_time = time.time()
        sorting.merge_sort(data.copy())  # เรียกใช้ฟังก์ชัน merge_sort จากโมดูล sorting
        ...
        start_time = time.time()
        sorting.counting_sort(data.copy())  # เรียกใช้ฟังก์ชัน counting_sort จากโมดูล sorting
    ...
    plt.title('Performance Comparison of Sorting Algorithms (Data 0-999999). Created By Supattra Pankulab')
    plt.xlabel('Input Size')
    plt.ylabel('Time (Milliseconds)')
    plt.legend()
    plt.grid(True)
    plt.savefig('Performance_Comparison_Shell_Quick_Merge_Radix_Counting_999999.png')
    plt.show()
