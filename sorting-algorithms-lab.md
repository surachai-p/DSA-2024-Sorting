...
```python
    ...
    for i in range(n):
        swapped = False
        last_swapped_index = 0  # ตำแหน่งสุดท้ายที่มีการสลับ
        print(f"\nรอบที่ {i+1}:")
        ...
        for j in range(0, n - i - 1):
            ...
            if arr[j] > arr[j + 1]:
                arr[j], arr[j + 1] = arr[j + 1], arr[j]
                swapped = True
                last_swapped_index = j  # บันทึกตำแหน่งที่สลับล่าสุด
                print(f"สลับ: {arr}")
            else:
                print("ไม่สลับ")
        ...
        if not swapped:
            ...
            break  # หยุดการทำงานเมื่อเรียงลำดับแล้ว

        # ลดขนาดช่วงที่ต้องตรวจสอบในรอบถัดไป
        n = last_swapped_index + 1
...
### บันทึกผลแบบทดสอบ
```html
   มีการเปรียบเทียบข้อมูลในรอบที่ 1 แต่ไม่มีการสลับข้อมูลเพราะข้อมูลเรียงลำดับแล้ว
```
บันทึกรูปผลแบบทดสอบ
...
```python
    def insertion_sort_desc_with_steps(arr):
    print(f"เริ่มต้น: {arr}")
    ...
    for i in range(1, len(arr)):
        ...
        while j >= 0 and arr[j] < key:  # เปลี่ยนเงื่อนไขเป็น < เพื่อเรียงจากมากไปน้อย
            arr[j+1] = arr[j]
            j -= 1
            print(f"  ย้าย {arr[j+2]} ไปทางขวา: {arr}")
...
### บันทึกผลแบบทดสอบ
```html
 อัลกอริทึมยังคงรักษาค่าที่ซ้ำกันไว้ และเรียงลำดับตามหลักการ Insertion Sort
เรียงจากมากไปน้อยถูกต้อง ตัวเลขที่มากที่สุดขึ้นก่อนและตัวเลขที่น้อยที่สุดอยู่ท้ายสุด
```
บันทึกรูปผลแบบทดสอบ
...
```python
    ...
    for i in range(n):
        max_idx = i  # ตำแหน่งของค่ามากที่สุดในรอบนี้
        print(f"\nรอบที่ {i+1}:")
        print(f"  ข้อมูลปัจจุบัน: {arr}")
        ...
        for j in range(i+1, n):
            ...
            if arr[j] > arr[max_idx]:  # เปลี่ยนเงื่อนไขเป็น "มากกว่า"
                max_idx = j
...
[90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 100]

### บันทึกผลแบบทดสอบ
```html
 ข้อมูลเกือบเรียงลำดับแล้วช่วยให้การสลับตำแหน่งลดลงมีเพียงค่า 100 และ 90 ที่ต้องย้ายไปตำแหน่งที่ถูกต้องหลังจากการสลับไม่กี่ครั้งอัลกอริทึมก็เสร็จสิ้นแต่มีจำนวนรอบเพิ่มมากขึ้น
```
บันทึกรูปผลแบบทดสอบ
...
test_data = [24, 34, 24, 12, 22, 24, 45, 24, 6, 24]
### บันทึกผลแบบทดสอบ
บันทึกโค้ดและรูปผลแบบทดสอบ
```html
 สามารถจัดการกับชุดข้อมูลที่มีค่าซ้ำกันได้ดี เนื่องจากแบ่งข้อมูลออกเป็น 3 ส่วน
left เก็บค่าที่น้อยกว่า pivot
middle เก็บค่าที่เท่ากับ pivot
right เก็บค่าที่มากกว่า pivot
วิธีนี้ช่วยลดปัญหาค่าซ้ำกันที่อาจทำให้ Quick Sort ทำงานไม่ดี เช่น กรณีที่ข้อมูลมีค่าซ้ำจำนวนมาก
```
...
1. เปรียบเทียบประสิทธิภาพกับ Insertion Sort ปกติเมื่อทดสอบกับชุดข้อมูลขนาดใหญ่
   ### บันทึกผลแบบทดสอบ
```html
 Shell Sort จะใช้เวลาน้อยกว่า Insertion Sort อย่างมาก Insertion Sort ใช้เวลานาน เพราะต้องเลื่อนค่าจำนวนมาก
ความแตกต่างชัดเจนขึ้นเมื่อข้อมูลมีขนาดใหญ่
```
บันทึกรูปผลแบบทดสอบ
...
### บันทึกผลแบบทดสอบ
```html
 Shell Sort เร็วกว่ามาก โดยเฉพาะเมื่อข้อมูลมีขนาดใหญ่แต่ Insertion Sort เร็วกว่าในข้อมูลเกือบเรียงลำดับแล้ว
```
บันทึกรูปผลแบบทดสอบ
...
### บันทึกผลแบบทดสอบ
```html
ข้อมูลถูกจัดเรียงจากน้อยไปมาก เห็นขั้นตอนของ Merge Sortเวลาที่ใช้ถูกวัดเป็น มิลลิวินาที
```
บันทึกรูปผลแบบทดสอบ
...
```html
ในการทดลองเปรียบเทียบประสิทธิภาพของอัลกอริทึมการเรียงลำดับ (Sorting Algorithms) หลายแบบทั้งในกรณีข้อมูลขนาดเล็กและขนาดใหญ่ พบว่า

การเรียงลำดับด้วย Bubble Sort, Insertion Sort, และ Selection Sort

ทั้งสามอัลกอริทึมนี้มีประสิทธิภาพที่ค่อนข้างต่ำเมื่อเทียบกับอัลกอริทึมที่มีประสิทธิภาพสูงกว่า (เช่น Quick Sort หรือ Merge Sort) โดยจะเห็นได้ว่าตัว Bubble Sort ใช้เวลานานมากในกรณีที่ข้อมูลมีขนาดใหญ่
Bubble Sort เป็นอัลกอริทึมที่มีประสิทธิภาพไม่ดีมากที่สุดเมื่อเปรียบเทียบกับ Insertion Sort และ Selection Sort ซึ่งมีเวลาในการประมวลผลที่ดีกว่าเล็กน้อย
Insertion Sort มีความเร็วที่ดีกว่าในกรณีที่ข้อมูลเกือบเรียงลำดับแล้ว หรือข้อมูลมีขนาดเล็ก แต่เมื่อข้อมูลขยายออกไปหรือมีความยุ่งเหยิงมากขึ้น ก็จะเห็นผลลัพธ์การประมวลผลที่ช้า
Selection Sort ก็ไม่ค่อยดีในการจัดการกับข้อมูลที่มีขนาดใหญ่ เนื่องจากต้องทำการค้นหาค่าต่ำสุดในทุกๆ รอบการวนลูป

การเรียงลำดับด้วย Shell Sort, Quick Sort, Merge Sort, Radix Sort, และ Counting Sort

Shell Sort เป็นอัลกอริทึมที่พัฒนาให้ดีขึ้นจาก Insertion Sort ด้วยการลดช่องว่างการเปรียบเทียบก่อน ทำให้มีประสิทธิภาพดีกว่าในการเรียงข้อมูลบางประเภท
Quick Sort เป็นอัลกอริทึมที่ทำงานได้ดีมาก โดยเฉพาะเมื่อข้อมูลมีขนาดใหญ่มาก การเลือก pivot อย่างมีประสิทธิภาพสามารถลดเวลาการประมวลผลได้มาก
Merge Sort ก็เป็นอัลกอริทึมที่มีประสิทธิภาพดีมาก และสามารถรองรับข้อมูลขนาดใหญ่ได้เป็นอย่างดี โดยมีความเร็วการทำงานที่มีความสม่ำเสมอในทุกกรณี
Radix Sort ใช้หลักการที่แตกต่างออกไปจากอัลกอริทึมอื่นๆ โดยการใช้การจัดเรียงตามหลักของแต่ละตัวเลข ซึ่งสามารถใช้เวลาในการประมวลผลได้อย่างรวดเร็วเมื่อข้อมูลมีลักษณะเหมาะสม
Counting Sort เป็นอัลกอริทึมที่มีประสิทธิภาพสูงเมื่อข้อมูลที่ใช้มีลักษณะเป็นจำนวนที่มีช่วงจำกัดและไม่เกินค่าที่กำหนด

การเปลี่ยนแปลงขนาดข้อมูล

เมื่อข้อมูลขนาดเล็ก (0-999) ถูกใช้ในการทดสอบ จะเห็นว่าอัลกอริทึมที่มีประสิทธิภาพต่ำ (Bubble Sort, Insertion Sort) ใช้เวลานานขึ้นเมื่อข้อมูลมีขนาดใหญ่ขึ้น
ในขณะที่ Quick Sort, Merge Sort, Radix Sort, และ Counting Sort ทำงานได้เร็วกว่าและสามารถรองรับขนาดข้อมูลที่ใหญ่มากได้โดยไม่เสียประสิทธิภาพมากนัก
ข้อมูลที่มีช่วงค่ามาก เช่น 0-99999 หรือ 0-999999 ส่งผลให้อัลกอริทึมที่ทำงานช้าอย่าง Bubble Sort หรือ Insertion Sort ทำงานได้ช้ามาก ส่วนอัลกอริทึมที่มีประสิทธิภาพสูงอย่าง Quick Sort หรือ Merge Sort สามารถจัดการได้ดีแม้ข้อมูลจะขยายออกไป

การเปรียบเทียบกับข้อมูลขนาดใหญ่

สำหรับข้อมูลที่มีขนาดใหญ่หรือมีความหลากหลายของข้อมูลมาก (เช่น ขนาดข้อมูลถึง 999999) อัลกอริทึมที่มีประสิทธิภาพสูง เช่น Quick Sort, Merge Sort, Radix Sort และ Counting Sort จะทำงานได้ดีและมีเวลาในการประมวลผลที่ต่ำ
ในขณะที่ Bubble Sort, Insertion Sort, และ Selection Sort จะทำให้ประสิทธิภาพโดยรวมลดลงมาก โดยเฉพาะในกรณีข้อมูลที่มีขนาดใหญ่มาก
    สำหรับข้อมูลขนาดเล็กหรือที่มีการจัดเรียงอยู่แล้ว อัลกอริทึมที่มีประสิทธิภาพต่ำอาจใช้เวลาน้อยและเหมาะสม เช่น Insertion Sort และ Selection Sort
