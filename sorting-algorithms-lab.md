...
# ทดสอบกับชุดข้อมูลที่เกือบเรียงลำดับแล้ว
nearly_sorted_data = [90, 64, 45, 34, 25, 24, 22, 12, 11, 6]
print("\nทดสอบกับชุดข้อมูลที่เกือบเรียงลำดับแล้ว:", nearly_sorted_data)

start_time = time.time()
sorted_nearly_sorted = selection_sort_desc(nearly_sorted_data.copy())
end_time = time.time()

print("ข้อมูลหลังเรียง:", sorted_nearly_sorted)
print(f"เวลาที่ใช้: {(end_time - start_time)*1000:.6f} มิลลิวินาที")
```
บันทึกรูปผลแบบทดสอบ
...
def quick_sort(arr):
    ...
    pivot = pivot_candidates[1]  # ค่า median เป็น pivot
...
start_time = time.time()
sorted_nearly_sorted = quick_sort(nearly_sorted_data.copy())
end_time = time.time()
...
duplicate_data = [5, 3, 8, 3, 3, 7, 8, 5, 5, 7, 3, 8, 5]
print("\nทดสอบกับชุดข้อมูลที่มีค่าซ้ำกันจำนวนมาก:", duplicate_data)

start_time = time.time()
sorted_duplicate_data = quick_sort(duplicate_data.copy())
end_time = time.time()

print("ข้อมูลหลังเรียง:", sorted_duplicate_data)
print(f"เวลาที่ใช้: {(end_time - start_time)*1000:.6f} มิลลิวินาที")
```
...
1. เปรียบเทียบประสิทธิภาพกับ Insertion Sort ปกติเมื่อทดสอบกับชุดข้อมูลขนาดใหญ่
   ### บันทึกผลแบบทดสอบ
```html
 โค้ดนี้เป็นการทดสอบประสิทธิภาพของอัลกอริธึมเรียงลำดับ 3 แบบ ได้แก่ Quick Sort, Insertion Sort, และ Shell Sort
```
บันทึกรูปผลแบบทดสอบ
...
### บันทึกผลแบบทดสอบ
```html
 โค้ดนี้เป็นการเปรียบเทียบประสิทธิภาพของอัลกอริธึมการเรียงลำดับ 3 แบบ ได้แก่ Quick Sort, Insertion Sort และ Shell Sort โดยทดสอบกับชุดข้อมูลที่แตกต่างกัน
```
บันทึกรูปผลแบบทดสอบ
...
### บันทึกผลแบบทดสอบ
```html
 โค้ดนี้เป็นการ Merge Sort พร้อมแสดงขั้นตอนการทำงานโดยละเอียด และวัดเวลาที่ใช้ในการทำงาน
```
บันทึกรูปผลแบบทดสอบ
...
```html
1. ข้อมูลขนาดเล็ก (1000, 5000, 10000)
Shell Sort: จะช้ากว่าการจัดเรียงแบบอื่น ๆ เล็กน้อย เนื่องจากเป็นอัลกอริธึมที่มีเวลาเฉลี่ย O(n^2) สำหรับกรณีที่ข้อมูลไม่สามารถแบ่งได้ดี
Quick Sort: เป็นอัลกอริธึมที่มีประสิทธิภาพสูงมากและมักทำงานได้เร็วในข้อมูลขนาดเล็ก โดยมีเวลาเฉลี่ย O(n log n)
Merge Sort: มักจะทำงานได้ดีแต่จะช้ากว่า Quick Sort เล็กน้อยเนื่องจากต้องใช้พื้นที่สำรอง (extra space) สำหรับการรวมข้อมูล
Radix Sort: จะมีความเร็วสูงเมื่อใช้กับข้อมูลที่เป็นตัวเลขและมีความแตกต่างน้อย (เช่น 0-99999) เพราะมีเวลา O(nk) ที่เหมาะสมกับการเรียงข้อมูลที่มีจำนวนหลักไม่เยอะ
Counting Sort: เป็นการเรียงข้อมูลที่มีประสิทธิภาพสูงเมื่อใช้กับข้อมูลที่มีช่วงค่าที่ไม่ใหญ่มาก โดยมีเวลา O(n+k)

2. ข้อมูลขนาดใหญ่ (20000, 40000, 100000)
Shell Sort: ยังคงช้าเมื่อเทียบกับอัลกอริธึมอื่น ๆ โดยมีประสิทธิภาพลดลงเมื่อขนาดข้อมูลเพิ่มขึ้น
Quick Sort: ยังคงทำงานได้ดีเมื่อขนาดข้อมูลใหญ่ โดยมีเวลาเฉลี่ย O(n log n) แต่หากมีการเลือก pivot ที่ไม่ดีอาจเกิดกรณี worst-case (O(n^2))
Merge Sort: จะทำงานช้าเล็กน้อยเนื่องจากต้องใช้พื้นที่ในการแบ่งข้อมูลแต่ยังคงประสิทธิภาพดี (O(n log n)) และเวลาไม่ขึ้นอยู่กับการจัดการ pivot
Radix Sort: เหมาะสมกับข้อมูลที่มีช่วงค่าคงที่และไม่ใหญ่มาก แต่จะมีเวลา O(nk) ซึ่งขึ้นอยู่กับจำนวนหลักและช่วงของข้อมูล ถ้าข้อมูลมีค่าแตกต่างมาก หรือมีจำนวนหลักมาก อาจจะช้ากว่าอัลกอริธึมเปรียบเทียบ
Counting Sort: ทำงานได้ดีที่สุดในข้อมูลที่ช่วงค่าสั้น ๆ การใช้เวลาจะเป็น O(n+k) ซึ่งเหมาะกับข้อมูลที่มีค่าคงที่และช่วงไม่ใหญ่
``` 
