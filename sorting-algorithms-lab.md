...
# วัดเวลาการทำงานของฟังก์ชัน bubble_sort
start_time = time.time()
...
Cell In[1], line 8
      6 # วัดเวลาการทำงานของฟังก์ชัน bubble_sort
      7 start_time = time.time()
----> 8 bubble_sort(sorted_data.copy())
      9 end_time = time.time()
     11 # แสดงเวลาที่ใช้ในการทำงาน
```
บันทึกรูปผลแบบทดสอบ
...
def insertion_sort_with_steps(arr):
    ...
    for i in range(1, len(arr)):
        ...
        # เปลี่ยนเงื่อนไขเพื่อการเรียงจากมากไปน้อย
        while j >= 0 and arr[j] < key:
            arr[j+1] = arr[j]
            j -= 1
            print(f"  ย้าย {arr[j+2]} ไปทางขวา: {arr}")
...
```def selection_sort_descending(arr):
    ...
    for i in range(n):
        max_idx = i  # เปลี่ยนจาก min_idx เป็น max_idx
        print(f"\nรอบที่ {i+1}:")
        print(f"  ข้อมูลปัจจุบัน: {arr}")
        ...
        for j in range(i+1, n):
            ...
            if arr[j] > arr[max_idx]:  # เปรียบเทียบหาค่ามากที่สุดแทนค่าต่ำสุด
                max_idx = j
...
เวลา (Time Complexity): Merge Sort จะทำงานใน O(n log n) ไม่ว่าจะเป็นข้อมูลที่เรียงแล้วหรือไม่เรียง โดยจะมีการแบ่งข้อมูลและรวมกลับไปเสมอ
พื้นที่ (Space Complexity): เนื่องจาก Merge Sort ใช้พื้นที่ในการจัดเก็บข้อมูลเพิ่มเติม (โดยการสร้างอาร์เรย์ชั่วคราวระหว่างการรวม) ดังนั้นจะใช้ O(n) ของพื้นที่
```
บันทึกรูปผลแบบทดสอบ
...
```html
เมื่อทำการเปรียบเทียบประสิทธิภาพของอัลกอริธึมการเรียงข้อมูลที่คุณกล่าวถึง เช่น Bubble Sort, Insertion Sort, Selection Sort, Shell Sort, Quick Sort, Radix Sort, และ Counting Sort ในสถานการณ์ต่างๆ ดังนี้:

1. กรณีข้อมูลขนาดเล็ก
เมื่อใช้กับชุดข้อมูลที่มีขนาดเล็ก (เช่น 100 หรือ 500 ข้อมูล):

Bubble Sort, Insertion Sort, และ Selection Sort ทำงานได้เร็วพอสมควรเพราะเวลาที่ใช้ในการจัดการกับข้อมูลที่มีขนาดเล็กไม่มากนัก แม้ว่าจะมีประสิทธิภาพไม่ดีเมื่อขนาดข้อมูลเพิ่มขึ้น แต่ในขนาดเล็กนี้ยังไม่เห็นความแตกต่างชัดเจน
Shell Sort และ Quick Sort จะทำงานได้ดีกว่าอัลกอริธึมแบบอื่นๆ อย่างชัดเจนในกรณีที่ขนาดข้อมูลเพิ่มขึ้นเล็กน้อย
Radix Sort และ Counting Sort ในกรณีข้อมูลที่มีค่าจำนวนน้อยๆ อาจจะทำงานได้ดี แต่ยังไม่เห็นความแตกต่างที่ชัดเจน
2. กรณีข้อมูลขนาดใหญ่
เมื่อใช้กับชุดข้อมูลที่มีขนาดใหญ่ (เช่น 5000, 10000, 20000):

Bubble Sort, Insertion Sort, และ Selection Sort ทำงานได้ช้าลงอย่างมากเมื่อข้อมูลขนาดใหญ่ขึ้น เนื่องจากทั้งสามอัลกอริธึมนี้มี เวลาในการทำงาน O(n^2) ทำให้ประสิทธิภาพลดลงอย่างมากเมื่อขนาดข้อมูลเพิ่มขึ้น
Shell Sort จะมีการปรับปรุงการทำงานที่ดีกว่าในกรณีนี้ เพราะใช้วิธีการลดช่องว่างในการเปรียบเทียบข้อมูล ทำให้เวลาการทำงานโดยรวมลดลงได้
Quick Sort และ Radix Sort จะทำงานได้เร็วมาก โดย Quick Sort จะมีเวลาในการทำงานที่ O(n log n) และ Radix Sort จะทำงานในเวลา O(nk) ซึ่ง k คือจำนวนหลักของตัวเลข
Counting Sort มักจะทำงานได้ดีเมื่อข้อมูลที่มีค่าจำนวนน้อยๆ และค่อนข้างจำกัดในช่วงที่กำหนด
3. กรณีข้อมูลที่มีความแตกต่างของข้อมูลน้อย (ข้อมูลเกือบเรียงลำดับแล้ว)
เมื่อข้อมูลมีลักษณะเป็น ข้อมูลที่เกือบเรียงลำดับแล้ว (หรือข้อมูลที่มีความแตกต่างน้อย):

Bubble Sort และ Insertion Sort จะทำงานได้ค่อนข้างดีในกรณีนี้ เพราะมี Best Case คือ O(n) เมื่อข้อมูลเกือบเรียงแล้ว
Selection Sort จะทำงานไม่ต่างจากกรณีทั่วไป เพราะมันไม่สนใจว่าอาร์เรย์จะเรียงลำดับอยู่แล้วหรือไม่
Shell Sort จะยังคงปรับปรุงเวลาได้ดีขึ้นกว่าทั้ง Bubble Sort และ Insertion Sort
Quick Sort จะยังคงทำงานได้ดี แต่บางครั้งอาจจะเลือก Pivot ที่ไม่ดีเมื่อข้อมูลเกือบเรียงแล้วทำให้เกิดการแบ่งข้อมูลที่ไม่สมดุล
Radix Sort และ Counting Sort จะทำงานได้ดีเช่นกันในกรณีนี้ หากข้อมูลอยู่ในช่วงที่จำกัดและไม่ซับซ้อนเกินไป
4. กรณีข้อมูลที่มีความแตกต่างของข้อมูลมาก (ข้อมูลสุ่ม)
เมื่อข้อมูลมีความ แตกต่างมาก (ข้อมูลที่มีการกระจายตัวอย่างกว้างขวาง เช่น ข้อมูลสุ่ม):

Bubble Sort, Insertion Sort, และ Selection Sort จะทำงานได้ช้าในกรณีนี้ เพราะการเปรียบเทียบและการสลับจะต้องทำหลายรอบ
Shell Sort จะทำงานได้ดีกว่าในกรณีนี้ เพราะการปรับลดช่องว่างในการเปรียบเทียบจะช่วยลดจำนวนการเปรียบเทียบที่ไม่จำเป็น
Quick Sort จะทำงานได้เร็วมาก เพราะมันมีการแบ่งข้อมูลเป็นสองส่วนที่ไม่ซ้ำกัน จึงสามารถดำเนินการกับข้อมูลที่มีความแตกต่างมากได้อย่างมีประสิทธิภาพ
Radix Sort จะทำงานได้ดีถ้าข้อมูลที่มีค่าผลรวมไม่มากและจำกัดในช่วงที่สามารถใช้การเรียงตามหลักตัวเลขได้
Counting Sort เหมาะกับกรณีนี้ถ้าค่าของข้อมูลมีขอบเขตจำกัดและไม่เป็นจำนวนมาก
สรุปผล:
